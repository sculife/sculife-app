...
//         {searchResult.length === 0 && (<View><Text>没有找到您要的东西</Text></View>)}
...
export default function SearchModalScreen() {
  ...
  const search = async () => {
    try {
      const res = await axios.get(
        Url.BASE_URL + '/api/posts/search?q=' + searchText,
        {
          headers: {
  ...
  return (
    <ScrollView>
      <View className="flex-1 items-center">
        <Text className="text-xl font-bold mt-5">Sculife Search</Text>
        <View className="w-[100%] flex-row items-center justify-center">
          <TextInput
            className="bg-slate-100 dark:bg-zinc-800 text-white w-[80%] m-5 mx-0 ml-2 py-3 px-5 rounded-md"
            placeholder="Search"
            placeholderTextColor={Colors.zinc['800+150']}
            onSubmitEditing={search}
            onChangeText={(text) => setSearchText(text)}
          ...
          searchResult.map((post, i) => (
            ...
            <View key={i} className="mx-7 my-2 w-[90%]">
              <TouchableOpacity
                ...
                // className="w-[100%] border dark:border-0 border-gray-300 rounded-lg p-4 dark:bg-zinc-800"
              >
                {/* TODO: backend */}
                {/* <AnnouncementCardView
                  post={post}
