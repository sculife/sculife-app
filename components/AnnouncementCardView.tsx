...
export default function AnnouncementCardView({
  ...
  return (
    <View className="bg-[#222] rounded-2xl px-5 py-3">
      ...
      {/* content */}
      <Text className="pb-4 pt-0" style={{fontSize:12}}>{handleText(content, 100)}</Text>
      {/* tags */}
      <View className='bg-[#222]'>
        <Text className='font-medium'>Important Announcements</Text> {/* TODO: should i need this? */}
      </View>
      {/* post info */}
      <View className="bg-[#222] flex-row justify-between">
        {/* author */}
        <View className="bg-[#222]">
          <Text className="font-bold text-gray-300">{handleText(authorName, 25)}</Text>
        </View>
        {/* time */}
